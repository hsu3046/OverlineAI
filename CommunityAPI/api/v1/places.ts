...
export default createGETHandler(async (url) => {
  ...
  const radius = url.searchParams.has("radius")
    ? numberQuery(url, "radius", { minimum: 500, maximum: 20_000 })
    : 5_000;
  const kind = enumQuery(url, "kind", ["all", "bookstore", "library"] as const, "all");
  const apiKey = requiredEnvironment("KAKAO_REST_API_KEY");
  const items = await searchKakaoPlaces(latitude, longitude, radius, kind as PlaceKind | "all", apiKey);
  const body: ListResponse<CommunityPlace> = { items, fetchedAt: new Date().toISOString() };
