...
struct OCRValidationRecord: Identifiable, Hashable, Codable {
    var id: UUID
    var createdAt: Date
    var testedLineCount: Int
    var correctionCount: Int
    var sampleTitle: String
    var isKoreanBookSample: Bool
    var note: String
    ...
    init(
        ...
        correctionCount: Int,
        sampleTitle: String = "",
        isKoreanBookSample: Bool = false,
        note: String = ""
    ) {
        self.id = id
        self.createdAt = createdAt
        self.testedLineCount = testedLineCount
        self.correctionCount = correctionCount
        self.sampleTitle = sampleTitle
        self.isKoreanBookSample = isKoreanBookSample
        self.note = note
    }

    var accuracy: Double {
        guard testedLineCount > 0 else { return 0 }
        let correctCount = max(testedLineCount - correctionCount, 0)
        return Double(correctCount) / Double(testedLineCount)
    }

    var passesM0: Bool {
        isKoreanBookSample && testedLineCount > 0 && accuracy >= 0.90
    }

    var accuracyPassesThreshold: Bool {
        testedLineCount > 0 && accuracy >= 0.90
    }

    var sampleLabel: String {
        sampleTitle.trimmed.isEmpty ? "샘플 미기록" : sampleTitle.trimmed
    }

    private enum CodingKeys: String, CodingKey {
        case id
        case createdAt
        case testedLineCount
        case correctionCount
        case sampleTitle
        case isKoreanBookSample
        case note
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        testedLineCount = try container.decodeIfPresent(Int.self, forKey: .testedLineCount) ?? 0
        correctionCount = try container.decodeIfPresent(Int.self, forKey: .correctionCount) ?? 0
        sampleTitle = try container.decodeIfPresent(String.self, forKey: .sampleTitle) ?? ""
        isKoreanBookSample = try container.decodeIfPresent(Bool.self, forKey: .isKoreanBookSample) ?? false
        note = try container.decodeIfPresent(String.self, forKey: .note) ?? ""
    }
}

enum OCRValidationStore {
    private static let key = "overline.ocrValidation.records.v1"

    static func load() -> [OCRValidationRecord] {
        guard
            let data = UserDefaults.standard.data(forKey: key),
            let records = try? JSONDecoder().decode([OCRValidationRecord].self, from: data)
        else {
            return []
        }
    ...
    static func add(_ record: OCRValidationRecord) {
        let records = [record] + load()
        save(records)
    }

    private static func save(_ records: [OCRValidationRecord]) {
