// Adapted from Supertone Supertonic at commit
// 7e2804f96016a7028cb1ed627353c61c1e9dd281 (MIT License).
import Foundation
import Accelerate
...
nonisolated let AVAILABLE_LANGS = ["en", "ko", "ja", "ar", "bg", "cs", "da", "de", "el", "es", "et", "fi", "fr", "hi", "hr", "hu", "id", "it", "lt", "lv", "nl", "pl", "pt", "ro", "ru", "sk", "sl", "sv", "tr", "uk", "vi", "na"]

nonisolated func isValidLang(_ lang: String) -> Bool {
...
nonisolated struct VoiceStyleData: Codable {
...
nonisolated final class UnicodeProcessor {
    let indexer: [Int64]
...
nonisolated func preprocessText(_ text: String, lang: String) -> String {
...
nonisolated func lengthToMask(_ lengths: [Int], maxLen: Int? = nil) -> [[[Float]]] {
...
nonisolated func getTextMask(_ textIdsLengths: [Int]) -> [[[Float]]] {
    let maxLen = textIdsLengths.max() ?? 0
    return lengthToMask(textIdsLengths, maxLen: maxLen)
}

nonisolated func sampleNoisyLatent(duration: [Float], sampleRate: Int, baseChunkSize: Int, chunkCompress: Int, latentDim: Int) -> (noisyLatent: [[[Float]]], latentMask: [[[Float]]]) {
...
nonisolated func getLatentMask(_ wavLengths: [Int64], _ cfgs: Config) -> [[[Float]]] {
...
nonisolated func writeWavFile(_ filename: String, _ audioData: [Float], _ sampleRate: Int) throws {
...
nonisolated let MAX_CHUNK_LENGTH = 300
nonisolated let ABBREVIATIONS = [
    "Dr.", "Mr.", "Mrs.", "Ms.", "Prof.", "Sr.", "Jr.",
    "St.", "Ave.", "Rd.", "Blvd.", "Dept.", "Inc.", "Ltd.",
    "Co.", "Corp.", "etc.", "vs.", "i.e.", "e.g.", "Ph.D."
]

nonisolated func chunkText(_ text: String, maxLen: Int = 0) -> [String] {
...
nonisolated func splitSentences(_ text: String) -> [String] {
...
nonisolated func timer<T>(_ name: String, _ f: () throws -> T) rethrows -> T {
    let start = Date()
    print("\(name)...")
    let result = try f()
...
nonisolated func sanitizeFilename(_ text: String, maxLen: Int) -> String {
...
nonisolated func loadCfgs(_ onnxDir: String) throws -> Config {
...
nonisolated final class TextToSpeech {
    let cfgs: Config
...
nonisolated func loadVoiceStyle(_ voiceStylePaths: [String], verbose: Bool) throws -> Style {
...
nonisolated func loadTextToSpeech(_ onnxDir: String, _ useGpu: Bool, _ env: ORTEnv) throws -> TextToSpeech {
    if useGpu {
