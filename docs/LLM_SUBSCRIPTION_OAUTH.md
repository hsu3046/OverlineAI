...
앱 전역에서 하나의 `LLMSettingsStore`를 공유하므로 설정 화면에서 변경한 토큰과 모델이 캡처 및 글조각 편집 화면에도 즉시 반영된다. `401` 또는 `403` 응답은 현재 인증 또는 모델 접근 거부 상태로 저장된다. 토큰·API 키나 모델을 다시 설정하거나 같은 선택으로 `AI 연결 테스트`에 성공하면 거부 상태를 해제한다.
