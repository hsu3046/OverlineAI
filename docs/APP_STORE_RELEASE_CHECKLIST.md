...
2026-09-09 자동 테스트 34개 및 Release Archive 검증 완료. 사용자 승인 후 Xcode 자동 서명 갱신으로 위젯·App Groups 배포 프로파일 문제를 해결하고 IPA export 및 서명 검증을 완료했다. 17:03 KST에 1.0.2 (6) 업로드 성공 및 Apple 후속 처리 시작을 확인했다. ONNX dSYM 누락 경고는 기존과 동일하다. 실기기 최종 검증·심사 제출·출시는 남아 있다. [현재 검증 결과와 다음 작업](AppStore/RELEASE_1.0.2.md)을 참조한다. 아래 내용은 이전 배포 기록이다.

기준일: 2026년 9월 5일

## 1.0.1 유지보수 업데이트

사용자가 최초 출시 완료를 확인했다. 한국어 지원 설정을 반영한 Version 1.0.1 / Build 5의 자동 테스트 18개, Release Archive, 배포용 IPA 서명/최소 OS 검증을 완료하고 2026-09-05 09:18 KST 업로드에 성공했다. Apple 후속 처리 시작을 확인했으며 실기기 검증, 빌드 선택 가능 여부 확인, 심사 제출과 출시는 미실시다. ONNX dSYM 누락 경고는 기존과 동일하다. [변경 목록, 새로운 기능 문구, 테스트 및 등록 순서](AppStore/RELEASE_1.0.1.md)를 최신 배포 체크리스트로 사용한다. 아래 내용은 최초 심사 및 Build 4 업로드 당시의 기록을 보존한 것이다.

1.1의 Apple 온디바이스 AI와 다운로드형 모델 검증은 [이슈 #21](https://github.com/hsu3046/OverlineAI/issues/21)로 분리했으며 이번 패치에 포함하지 않는다.
